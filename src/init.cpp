...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "moonet.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "moonetd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the moonet and zmuu money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in moonet/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in moonet/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in moonet/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all moonet specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
//    strUsage += HelpMessageOpt("-preferredDenom=<n>", strprintf(_("Preferred Denomination for automatically minted Zerocoin  (1/5/10/50/100/500/1000/5000), 0 for no preference. default: %u)"), 0));
    strUsage += HelpMessageOpt("-backupzmoonet=<n>", strprintf(_("Enable automatic wallet backups triggered after each zmuu minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizemoonetamount=<n>     " + strprintf(_("Keep N moonet anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i-Hell Freezes The moonet Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_("")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that moonet is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    std::string strWalletFile = GetArg("-wallet", "muu.wlt.file");
#endif // ENABLE_WALLET
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. moonet Core is shutting down."));
...
#endif
    // Make sure only a single moonet process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. moonet Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        // Check if an old wallet exists or need to be created
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // moonet: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZmoonetMinted();
                        RecalculateZmoonetSpent();
                    }
                    RecalculatemoonetSupply(1);
                }
                ...
                // moonet: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of moonet Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart moonet Core to complete") << "\n";
        ...
        bool fEnableZmoonetBackups = GetBoolArg("-backupzmoonet", true);
        pwalletMain->setZmoonetAutoBackups(fEnableZmoonetBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
    ...
    if (nZeromintPercentage > 100) nZeromintPercentage = 0;
    if (nZeromintPercentage < 10) nZeromintPercentage = 0;
...
//    }
//
//    nAnonymizemoonetAmount = GetArg("-anonymizemoonetamount", 0);
//    if (nAnonymizemoonetAmount > 999999) nAnonymizemoonetAmount = 999999;
//    if (nAnonymizemoonetAmount < 2) nAnonymizemoonetAmount = 2;
    ...
    LogPrintf("Anonymize moonet Amount %d\n", nAnonymizemoonetAmount);
    ...
    /* Denominations
       ...
       For example:
       1moonet+1000 == (.1moonet+100)*10
       10moonet+10000 == (1moonet+1000)*10
    */

...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                tr("Warning: If you encrypt your wallet and lose your passphrase, you will <b>LOSE ALL OF YOUR muu</b>!") + "<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                muuGUI* window = static_cast<muuGUI*>(parentWidget());
...
bool AskPassphraseDialog::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn){
    muuGUI* gui = static_cast<muuGUI*>(parentWidget());
...
void AskPassphraseDialog::warningMessage() {
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("moonet will close now to finish the encryption process. "
               "Remember that encrypting your wallet cannot fully protect "
               "your muu from being stolen by malware infecting your computer.") +
            "<br><br><b>" +
...
void AskPassphraseDialog::backupPassword() {
    hide();
    static_cast<muuGUI*>(parentWidget())->showHide(true);

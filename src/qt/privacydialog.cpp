...
#include <QtWidgets>
#include <zmuu/deterministicmint.h>
                                                ...
                                                fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zmuu ought to be enough for anybody." - Bill Gates, 2017
    ui->zmuupayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzmuuSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zmuupayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzmuu_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zmuu is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zmuu, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zmuu...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zmuu in ") +
...
void PrivacyDialog::on_pushButtonSpendzmuu_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zmuu is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zmuu, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zmuu
        sendzmuu();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zmuu
    sendzmuu();
}

void PrivacyDialog::on_pushButtonZmuuControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZmuuControlDialog* zmuuControl = new ZmuuControlDialog(this);
    zmuuControl->setModel(walletModel);
    zmuuControl->exec();
}

void PrivacyDialog::setZmuuControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzmuuSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzmuu()
{
    ...
    double dAmount = ui->zmuupayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zmuu is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " muu </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zmuu</b>";
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zmuu was spent successfully update the addressbook with the label
    ...
    // Clear zmuu selector in case it was used
    ZmuuControlDialog::setSelectedMints.clear();
    ui->labelzmuuSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zmuu Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zmuu </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zmuu "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zmuu "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zmuu "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zmuu </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zmuu </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zmuu </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzmuuSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzmuu->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zmuu
        //ui->pushButtonMintzmuu->setEnabled(false);
        //ui->pushButtonMintzmuu->setToolTip(tr("zmuu is currently disabled due to maintenance."));

        // Spend zmuu
        ui->pushButtonSpendzmuu->setEnabled(false);
        ui->pushButtonSpendzmuu->setToolTip(tr("zmuu is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zmuu
        //ui->pushButtonMintzmuu->setEnabled(true);
        //ui->pushButtonMintzmuu->setToolTip(tr("PrivacyDialog", "Enter an amount of muu to convert to zmuu", 0));

        // Spend zmuu
        ui->pushButtonSpendzmuu->setEnabled(true);
        ui->pushButtonSpendzmuu->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}

...
#include <stdint.h>
#include <iostream>
#include "zTBP/deterministicmint.h"
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TBP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZTBPSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZTBP(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
    ...
    if(!isMine(addressTo)){
        receipt.SetStatus(_("To convert zTBP back to TBP the return address must be from your wallet"), 1);
        return false;
    }
...
static void NotifyzTBPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzTBPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzTBPReset.connect(boost::bind(NotifyzTBPReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzTBPReset.disconnect(boost::bind(NotifyzTBPReset, this));

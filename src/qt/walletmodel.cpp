...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 muu.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal moonet:URI (moonet:XyZ...?message=example)
            {
...
bool WalletModel::createZmuuSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZmuu(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
    ...
    if(!isMine(addressTo)){
        receipt.SetStatus(_("To convert zmuu back to muu the return address must be from your wallet"), 1);
        return false;
    }
...
static void NotifyzmuuReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzmuuReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzmuuReset.connect(boost::bind(NotifyzmuuReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzmuuReset.disconnect(boost::bind(NotifyzmuuReset, this));

...
// moonet: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_17_MASTERNODE_PAYMENT_CHECK) r = SPORK_17_MASTERNODE_PAYMENT_CHECK_DEFAULT;
        if (nSporkID == SPORK_18_NEW_PROTOCOL_FORK) r = SPORK_18_NEW_PROTOCOL_FORK_DEFAULT;
        if (nSporkID == SPORK_19_LOWERED_MASTERNODE_PAYMENT) r = SPORK_19_LOWERED_MASTERNODE_PAYMENT_DEFAULT;
        if (nSporkID == SPORK_20_REWARD_HALVING_START) r = SPORK_20_REWARD_HALVING_START_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_17_MASTERNODE_PAYMENT_CHECK") return SPORK_17_MASTERNODE_PAYMENT_CHECK;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_17_MASTERNODE_PAYMENT_CHECK) return "SPORK_17_MASTERNODE_PAYMENT_CHECK";

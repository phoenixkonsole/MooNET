...
//! -paytxfee default
static const CAmount DEFAULT_TRANSACTION_FEE = 0.000001 * COIN;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 moonet at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zmuu send
enum ZerocoinSpendStatus {
    Zmoonet_SPEND_OKAY = 0,                            // No error
    Zmoonet_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    Zmoonet_WALLET_LOCKED = 2,                         // Wallet was locked
    Zmoonet_COMMIT_FAILED = 3,                         // Commit failed, reset status
    Zmoonet_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    Zmoonet_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    Zmoonet_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    Zmoonet_TRX_CREATE = 7,                            // Everything related to create the transaction
    Zmoonet_TRX_CHANGE = 8,                            // Everything related to transaction change
    Zmoonet_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    Zmoonet_INVALID_COIN = 10,                         // Selected mint coin is not valid
    Zmoonet_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    Zmoonet_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    Zmoonet_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    Zmoonet_SPENT_USED_Zmoonet = 14,            // Coin has already been spend
    Zmoonet_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    Zmoonet_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
...
public:
    ...
    std::string ResetMintZerocoin(bool fExtendedSearch = false);
    std::string ResetSpentZerocoin();
    void ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored);
    void ZmoonetBackupWallet();
   ...
   int getZeromintPercentage()
    ...
    void setZmoonetAutoBackups(bool fEnabled)
    {
    ...
    bool GetMasternodeVinAndKeys(CTxIn& txinRet, unsigned int& tierRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash = "", std::string strOutputIndex = "");
    /// Extract txin information and keys from output
    bool GetVinAndKeysFromOutput(COutput out, CTxIn& txinRet, unsigned int& tierRet, CPubKey& pubKeyRet, CKey& keyRet);
    ...
    /** zdogec reset */
    boost::signals2::signal<void()> NotifyzmuuReset;
